...
@Observable
final class DefaultExpenseFormViewModel {
    var name: String = ""
    var payDay: Int = 1
    ...
    let original: DefaultExpense?
    private let store: DefaultExpenseStore

    init(item: DefaultExpense? = nil, store: DefaultExpenseStore) {
        self.original = item
        self.store = store
        if let item {
            name = item.name
